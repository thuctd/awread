...
export class ForgotPasswordGear {
  ...
  generateLink(token, userId, origin) {
    return `${origin ?? 'https://awread.vn'}/new-password;token=${token};userId=${userId}`;
  }
  ...
  async send({ email, origin }) {
    ...
    const generateLink = this.generateLink(token, user.userId, origin);
    const content = `
