import { Component, OnInit, ChangeDetectionStrategy, Input, ViewChild, ElementRef, forwardRef, ChangeDetectorRef, SimpleChanges, OnChanges, OnDestroy } from '@angular/core';
...
export class GenresField implements OnInit, ControlValueAccessor, OnChanges, OnDestroy {
  ...
  private workAroundKeepShowListAfterSelectOption() {
    setTimeout(() => {
      if (this.genreInput && this.genreInput.nativeElement && this.genres.length) {
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    const addedValue = this.mode === 'object' ? event.option.value.name : event.option.value;
    this.updateCurrentState(addedValue);
  }
  ...
  private getDuplicatedItems(selectedGenres: string[], item: any | string) {
    if (this.mode === 'object') {
      ...
      return selectedGenres && !!selectedGenres.filter(selectedItem => selectedItem === item.id).length;
    }
    return !!selectedGenres.filter(selectedItem => selectedItem.toLowerCase() === item.toLowerCase()).length;
  }
  ...
  getNameFromId(genre: string) {
    ...
    return this.mode === 'object' ? foundItem ? foundItem.name : genre : genre;
  }
}
