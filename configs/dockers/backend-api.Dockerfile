...
RUN --mount=type=bind,source=/root/.pnpm-store/v3,target=/root/.pnpm-store/v3|| echo "There were failing mount!"
RUN --mount=type=bind,source=/usr/local/bin/pnpm,target=/usr/local/bin/pnpm|| echo "There were failing mount!"
RUN --mount=type=bind,source=/usr/local/lib/node_modules,target=/usr/local/lib/node_modules|| echo "There were failing mount!"
WORKDIR /batcave
RUN npm i -g pnpm
